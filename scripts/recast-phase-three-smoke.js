...
// Give the Lounge a deterministic menu surface to enhance.
const menu=w.document.createElement('div');menu.className='start-panel';menu.innerHTML='<h1>Deep Sea Diver: Recast</h1><p class="sub">Dive deeper.</p>';w.document.body.appendChild(menu);
...
const btn=menu.querySelector('.recast-soundtrack-btn');
if(!btn||!/Soundtrack Lounge/.test(btn.textContent))throw new Error('Soundtrack Lounge menu button missing');
if(!w.RECAST_LOUNGE||typeof w.RECAST_LOUNGE.open!=='function')throw new Error('Lounge controller missing');
w.RECAST_LOUNGE.open();
const lounge=w.document.getElementById('recast-lounge');
if(!lounge)throw new Error('Soundtrack Lounge overlay did not open');
...
console.log('Recast Phase Three smoke passed: shiny palettes, Soundtrack Lounge, heard-track unlocks, White Squid mascot, and boss-theme families verified.');
dom.window.close();
process.exit(0);
